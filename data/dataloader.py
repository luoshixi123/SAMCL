...
class Unlabeld_RegDBData(data.Dataset):
    def __init__(self, data_dir, trial, transform, rgb_cluster=False, ir_cluster=False):
        ...
        self.rgb_cluster = rgb_cluster
        self.ir_cluster = ir_cluster
#从指定的路径加载训练图像和标签。整图像大小并将其转换为 NumPy 数组。保存图像数据、路径和标签。
    ...
    def __getitem__(self, index):
        if self.rgb_cluster:
            ...
            print('error getitem!')#根据索引 index 获取相应的 RGB 或 IR 图像、标签和路径。应用预处理转换 transform。返回图像数据、标签、路径和图像类型（RGB 或 IR）
    ...
    def __len__(self):
        if self.rgb_cluster:
            ...
            print('error len!')#返回数据集中 RGB 或 IR 图像的数量。
